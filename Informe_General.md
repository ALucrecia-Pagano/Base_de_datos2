...
**Parte A — Plan de indexado** sobre 5 consultas reales con `Seq Scan` (ranking de clientes, productos vs. promedio de categoría, top 3 por facturación mensual, productos por categoría y rango de precio). De los candidatos evaluados, 2 quedaron aplicados en firme (`idx_producto_categoria_precio_activo` sobre Q6, `idx_producto_categoria_precio` sobre Q2) y 2 se descartaron explícitamente por sobreindexación: un índice parcial ignorado por el planificador en 9/9 corridas por baja selectividad, y un segundo candidato redundante con una PK compuesta ya existente (`pk_detalle_pedido`), demostrado con un plan idéntico con y sin el candidato. Un caso adicional (B-tree sobre `fecha_hora` para Q4) tuvo un historial de idas y vueltas — descartado por corrida única, aceptado tras una medición de 3 rondas que nunca quedó archivada, y **descartado de nuevo** tras una auditoría externa que remidió con salida real archivada y encontró dirección inconsistente entre rondas (además de perjudicar a una tercera consulta, Q3, por pérdida de paralelismo); el índice fue eliminado en firme de la base de trabajo. Todo el proceso de idas y vueltas quedó documentado, no ocultado.

**Parte B — Vistas y seguridad por roles.** 6 vistas (`vistas.sql`): productos vigentes con categoría, ventas agregadas por cliente, pedidos con datos del cliente (fila a fila), pedidos con datos de usuario (vinculados por mail), detalle de pedido con nombre de producto, y una vista de seguridad (`v_usuario_publico`) que expone la tabla `usuario` sin la columna `contrasena`. Como el esquema heredado no tenía tabla de autenticación, se agregó `usuario` como tabla nueva sin tocar `cliente`, con datos de prueba versionados (incluidos los que matchean con clientes reales). Un rol de solo lectura (`tp5_reportes`, `NOLOGIN`) tiene `SELECT` sobre las 6 vistas y la vista materializada, pero **no** sobre las tablas base (verificado con un intento real de acceso denegado). Cada vista se verifica contra una consulta manual equivalente con `EXCEPT` en ambos sentidos y con `count(*)` (para detectar diferencias de filas duplicadas que `EXCEPT` por sí solo no distingue), en un script ejecutable que falla si alguna comparación no coincide (`verificacion_equivalencia.sql`).
