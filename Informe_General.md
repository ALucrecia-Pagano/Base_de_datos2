...
**Parte A — Plan de indexado** sobre 4 consultas reales con `Seq Scan` (ranking de clientes, productos vs. promedio de categoría, top 3 por facturación mensual y productos de una categoría en un rango de precio). Quedaron 2 índices aplicados en firme: el de Q6 (~41%) y el de Q2 (~37%, `Seq Scan` → `Bitmap Heap Scan`). Se descartaron el índice parcial de Q5 por sobreindexación (ignorado por el planificador en 9/9 corridas, por baja selectividad), el BRIN sobre `fecha_hora` (correlación física ~0) y el B-tree sobre `fecha_hora` de Q4: se había aceptado con ~8,9%, pero tras la devolución de la cátedra se remidió con 3 rondas archivadas, no mejoró de forma consistente (se perdía el paralelismo) y se descartó. Los cambios de conclusión quedaron documentados, no ocultados.
