...
| Qué propuso y qué se aceptó (Prueba 3) | La primera corrida sin calentamiento daba el resultado invertido (los índices "aceleraban" la escritura) por el arranque en frío de la Ronda 1. Con calentamiento: 12.4 → 18.3 ms (+47%), en las 3 rondas. Se aceptó la medición con calentamiento, archivando todas las rondas. Además se verificó con `EXPLAIN` que Q6 sigue usando su índice parcial con el de Q2 presente (`plan_q6_con_dos_indices.txt`) |
